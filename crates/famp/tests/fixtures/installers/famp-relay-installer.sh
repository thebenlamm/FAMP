...
if [ -n "${FAMP_RELAY_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$FAMP_RELAY_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${FAMP_RELAY_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$FAMP_RELAY_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${FAMP_RELAY_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${FAMP_RELAY_UNMANAGED_INSTALL:-}"
...
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.32.0"},"source":{"app_name":"famp-relay","name":"FAMP","owner":"thebenlamm","release_type":"github"},"version":"1.0.0"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/famp-relay"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/famp-relay"
fi
...
The installer for famp-relay 1.0.0
...
USAGE:
    famp-relay-installer.sh [OPTIONS]
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set FAMP_RELAY_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "famp-relay-aarch64-apple-darwin.tar.xz")
            _arch="aarch64-apple-darwin"
            _zip_ext=".tar.xz"
            _bins="famp-relay"
            _bins_js_array='"famp-relay"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            ...
            _updater_bin=""
            ;;
        "famp-relay-x86_64-apple-darwin.tar.xz")
            ...
            _updater_bin=""
            ;;
        "famp-relay-x86_64-unknown-linux-gnu.tar.xz")
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="famp-relay-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="famp-relay-x86_64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-apple-darwin")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="famp-relay-x86_64-unknown-linux-gnu.tar.xz"
...
install() {
    ...
    if [ -n "${FAMP_RELAY_INSTALL_DIR:-}" ]; then
        _force_install_dir="$FAMP_RELAY_INSTALL_DIR"
