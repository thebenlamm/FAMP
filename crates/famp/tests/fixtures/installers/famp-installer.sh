...
APP_NAME="famp"
APP_VERSION="1.0.0"
if [ -n "${FAMP_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$FAMP_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${FAMP_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$FAMP_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/thebenlamm/FAMP/releases/download/v1.0.0"
elif [ -n "${FAMP_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$FAMP_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/thebenlamm/FAMP/releases/download/v1.0.0"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/thebenlamm/FAMP/releases/download/v1.0.0"
fi
if [ -n "${FAMP_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$FAMP_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${FAMP_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$FAMP_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${FAMP_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$FAMP_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${FAMP_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${FAMP_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${FAMP_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.32.0"},"source":{"app_name":"famp","name":"FAMP","owner":"thebenlamm","release_type":"github"},"version":"1.0.0"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/famp"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/famp"
fi
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set FAMP_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "famp-aarch64-apple-darwin.tar.xz")
            ...
            _updater_bin=""
            ;;
        "famp-x86_64-apple-darwin.tar.xz")
            ...
            _updater_bin=""
            ;;
        "famp-x86_64-unknown-linux-gnu.tar.xz")
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="famp-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="famp-x86_64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-apple-darwin")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="famp-x86_64-unknown-linux-gnu.tar.xz"
...
install() {
    ...
    if [ -n "${FAMP_INSTALL_DIR:-}" ]; then
        _force_install_dir="$FAMP_INSTALL_DIR"
