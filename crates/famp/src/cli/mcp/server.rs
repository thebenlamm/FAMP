    ...
    serde_json::json!([
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "more_coming": { "type": "boolean", "description": "OPTIONAL, new_task mode only. Set true when this is the FIRST of multiple envelopes briefing the same task — the receiver will hold the task as 'pending follow-up' instead of treating it as ready to commit on the first envelope. Send subsequent context via famp_send mode=deliver, then a final mode=deliver (or repeat new_task with more_coming=false) when the briefing is complete. Default false (the task is fully briefed in this single envelope). Mirrors the body.interim flag on deliver envelopes. Ignored outside new_task mode." }
                },
                "required": ["peer", "mode"]
            }
        },
