...
/// arbitrary later point instead of an actionable handshake refusal.
///
/// ## Upgrading past a bump — FOUR steps, not two
///
/// `just install` + `famp daemon restart` is NOT sufficient. Every live
/// window re-registers after the restart, and mailboxes are durable per
/// name so nothing queued is lost — but two binaries `just install`
/// deliberately does not touch will be left on the old protocol:
///
/// 1. `just install` — moves `~/.cargo/bin/famp` only.
/// 2. `just install-gateway` — `just install` does NOT rebuild
///    `~/.cargo/bin/famp-gateway` (see `Justfile`, `install-gateway`).
///    A pre-bump gateway is rejected at Hello once the broker restarts,
///    so cross-host federation goes dark until this runs **on BOTH
///    hosts**. `just install-all` does steps 1 and 2 together.
/// 3. Redeploy the host Stop hook. `~/.claude/hooks/famp-await.sh` is
///    written ONLY by `famp install-claude-code`, and `just install`
///    deliberately does not run it (running it on a plugin-wired machine
///    registers a second MCP server — see the `install` recipe's comment).
///    On a plugin-wired machine the equivalent is `/plugin update famp@famp`,
///    which picks up `plugins/<host>/hooks/famp-await.sh`; those copies are
///    pre-rendered in-repo and regenerated by `scripts/gen-plugin.sh`.
/// 4. `famp daemon restart` — picks up the new broker.
