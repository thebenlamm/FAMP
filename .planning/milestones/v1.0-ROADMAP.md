...
- [x] **Phase 9: End-to-End Cross-Host Delivery** - Full bidirectional `request → commit → deliver → ack` task cycle across two machines through the gateway. (completed 2026-07-27; verified `passed`, `09-VERIFICATION.md`. Surfaced and fixed a real BUS-11 ingress compliance bug that had made every relayed envelope permanently unreadable on the receiving side.)
- [x] **Phase 10: Test Reactivation + Setup Docs** - Deferred federation tests triaged and green, a live two-process E2E in `just ci`, and a two-machine setup guide. (completed 2026-07-27; 27/27 parked `_deferred_v1/` tests triaged as RETIRE with a per-file rationale ledger, two compiled CI-gate guards pin the Phase 9 E2E into the default nextest set, `docs/GATEWAY-SETUP.md` gated by two binary-accuracy tests. `10-VERIFICATION.md` is `human_needed` and `10-HUMAN-UAT.md` `failed` on DOC-04's *unassisted-follower* clause only — that failure is what scoped Phase 11, and Phase 11's UAT-01 re-ran the same Gate A dogfood to a recorded PASS. Superseded, not open.)
