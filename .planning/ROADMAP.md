...
- [ ] **Phase 2: Minimal Task Lifecycle** — 5-state FSM (`REQUESTED → COMMITTED → {COMPLETED | FAILED | CANCELLED}`; 1 initial + 1 intermediate + 3 terminals), compile-time terminal enforcement via exhaustive enum `match`, proptest transition-legality tests (FSM-02 narrowed, FSM-03/04/05/08)
