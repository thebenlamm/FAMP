...
- [x] **CLI-04**: `famp await [--timeout <duration>]` blocks up to `<duration>` waiting for the next unread inbox entry; returns immediately if one is already waiting. Default timeout 60s. Returns structured output (task-id, peer, message class, body).
- [x] **CLI-05**: `famp inbox [--unread]` lists inbox entries without blocking. `--unread` filters to unread-only. Does not mark entries read.
