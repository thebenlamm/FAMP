...
## v1.0 Federation Profile — Gateway Core (Shipped: 2026-07-29)

**Phases completed:** 6 phases (7–12), 29 plans, 58 tasks
**Timeline:** 2026-07-23 → 2026-07-29 (7 days, 106 commits, 185 files, +21064/−4080)
**Requirements:** 29/29 satisfied (GW-01..04, LIVE-01/02, WIRE-01/02, TRUST-01/02, TEST-01..03, DOC-04/05, ADDR-01..03, OBS-01, SEC-01..04, UAT-01, REL-01..05)
**Release:** `v1.0.0` annotated + pushed at `5edff41`; 12/12 CI check-runs green at that exact SHA (0 failures), attested in `12-CI-ATTESTATION.md`
**Closeout:** `override_closeout` — 42 pre-existing open artifacts acknowledged and deferred (see STATE.md § Deferred Items)

**Delivered:** Gate A closed. An agent on one of Ben's machines exchanges signed FAMP envelopes with an agent on a second machine he controls, bidirectionally, driven by the shipping `famp` client — proven live macOS ↔ Linux over Tailscale on 2026-07-29, task `019fab97-…` reaching terminal COMPLETED on **both** hosts with `sig_verified: true`. `famp-gateway` (Layer 2) wraps the preserved `famp-transport-http` + `famp-keyring`, proxying remote principals onto the local UDS bus over signed HTTPS. See [milestones/v1.0-ROADMAP.md](milestones/v1.0-ROADMAP.md) · [milestones/v1.0-REQUIREMENTS.md](milestones/v1.0-REQUIREMENTS.md).

**Key accomplishments:**

- **Broker-liveness fork resolved with zero `famp-bus` change (Design A, Phase 7).** `ProxiedPrincipal::register` backs each remote principal with a no-spawn UDS `Register` carrying the *gateway's own* PID, so the broker's unmodified `kill(pid,0)` sweep can no longer reap a cross-host holder. Proven three ways: a unit test showing `register()` has no PID-uniqueness constraint (N clients on one PID survive and reap together), plus real-process SIGKILL/reap and cross-talk-isolation subprocess tests against a genuine `famp-gateway` OS process — not a `Broker<E>` double. LIVE-01/02, GW-04.
- **Signed cross-host wire with no local-bus wire break (Phase 8).** Seven optional, omit-when-empty federation fields (`from_domain`, `to_domain`, `sender_key_id`, `nonce`, `expiry`, `capability`, `approval`) ride the one existing INV-10 signature — local-bus bytes stay byte-identical. Two-machine TOFU trust bootstraps via `famp peer export --as <name>` → hand-copied 3-field blob → `famp peer import`, backed by a new generate-once-and-persist gateway keypair that closed a previously-nonexistent keygen gap; `verify_inbound` is the single pure trust decision (`peek_sender` → keyring lookup → `verify_strict`). WIRE-01/02, TRUST-01/02.
- **Live bidirectional relay + the BUS-11 bug it caught (Phase 9).** `run_ingress` (axum + rustls, `verify_inbound_any` as sole trust gate) and per-principal `run_egress` (Await → sign → POST drain loop) compose under one `tokio::select!` and a shared `Arc<Mutex<GatewayRegistry>>`. The two-process loopback E2E closing a full `request → commit → deliver → ack` cycle to a converged terminal FSM state on both sides surfaced and fixed a real BUS-11 compliance bug in the ingress path that had made **every** relayed envelope permanently unreadable on the receiving side. GW-01/02/03.
- **Parked test debt retired honestly, not reactivated (Phase 10).** All 27 `_deferred_v1/` federation tests triaged 27/27 RETIRE (0 reactivate) with a permanent per-file rationale ledger; two falsification-tested guards instead lock the real Phase 9 E2E into the default `cargo nextest run --workspace` set, and `docs/GATEWAY-SETUP.md` ships gated by two binary-accuracy tests that extract the guide's flags from the live CLI. TEST-01/02, DOC-04.
- **The Gate A dogfood found the actual blocker, and Phase 11 closed it.** No *shipping* client could address a remote principal — only a hand-written injector. `famp send --to agent:<domain>/<name>` now emits the exact domain-qualified envelope the gateway relays, with the class branched by send mode (`--new-task`→request, `--task`→commit, `--task --terminal`→deliver) so the FSM can actually terminate through the CLI/MCP surface, while `--to bob` stays byte-identical to before. One host-level federation authority (`--domain` > `FAMP_OWN_DOMAIN` > `$FAMP_HOME/own-domain`) makes the load-bearing "envelope `from` authority == peer-pinned label" invariant structural. All 8 dogfood findings in the setup guide corrected — including a wiring inversion the flag-grep gate had missed — with semantic assertions proven RED on the reintroduced inversion. ADDR-01..03, OBS-01, DOC-05, TEST-03, UAT-01.
- **Two trust-boundary holes found and fixed *before* `v1.0.0`, not after (Phase 11 plans 07–08).** The broker now binds a `Send`'s envelope `from` to the authenticated connection's effective identity — rejecting a forged `from` before any mailbox insertion, for both agent sends and channel posts — and the gateway rejects (never signs) any drained envelope whose `from` authority isn't its own. Ingress rejects envelopes not addressed to this gateway's own domain and mailbox; egress rejects client-supplied federation metadata; the route map is built only from explicit `--backs`/single-peer bindings and is startup-fatal fail-closed on any ambiguity or duplicate. SEC-01..04.
- **Release gate held under an independent adversarial pass (Phase 12).** A two-reviewer pass (self + backgrounded codex) over the shipped `v1.0.0-rc.1` trust boundary found and fixed a real timestamp-validation defect in `federation_format_ok` plus a silently-dropped gateway route, and triaged four other genuine-but-out-of-scope findings to documented-accept with source-grounded rationale — 10/10 findings dispositioned, zero untriaged. The fire-and-forget send-confirmation boundary is pinned across three surfaces (guide §6, `--help`, README) by one non-vacuous test, and §16's proposed limitation wording was **re-evaluated and dropped** rather than shipped stale, because ADDR-02 had since made remote sends FSM-driving. REL-01..05.

**Scope growth (honest record):** planned as Phases 7–10; shipped as 7–12. The two added phases were both discovery-driven, not scope creep — Phase 11 from the Gate A dogfood (no shipping client could reach a remote principal; 8 guide defects; a `from`-forgery hole), Phase 12 from design review C's §16 nine-item release checklist.

**Known deferred:** Phase 10's `10-VERIFICATION.md` remains `human_needed` and `10-HUMAN-UAT.md` `failed` on DOC-04's *unassisted-follower* clause. That failure is what scoped Phase 11, whose UAT-01 re-ran the same Gate A dogfood with the real client to a recorded PASS — superseded, not open. Gate B (conformance vector pack) stays independent and event-driven: it ships when a second implementer commits to interop.
