...
# Validate GitHub Actions workflow files. Catches a class local YAML parsing
# CANNOT: `${...}` expression syntax is substituted by Actions into run: blocks
# textually, ignoring shell comment syntax, so an empty expression -- even
# inside a `#` comment -- makes the whole file invalid. That surfaces only as a
# zero-job failed run with no logs, and python yaml.safe_load parses the file
# happily. A comment describing that very mechanism broke install-gate.yml once.
#
# .github/workflows/release.yml is EXCLUDED: it is generated by `dist generate`
# and must never be hand-edited, so its style-level findings are not actionable
# here. It is protected instead by `just check-installer-drift`, which
# regenerates and diffs it.
check-actionlint:
    #!/usr/bin/env bash
    set -euo pipefail
    if ! command -v actionlint >/dev/null 2>&1; then
        echo "ERROR: actionlint not found on PATH. Install with: brew install actionlint" >&2
        exit 1
    fi
    shopt -s nullglob
