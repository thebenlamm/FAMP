...
**v1.1 bus protocol bumps (`BUS_PROTO_VERSION`, QUAR-10):** v1.1 adds fail-closed provenance stamping to every mailbox record (Phase 14, inbound-content-is-DATA quarantine), which changes the wire shape of `famp_inbox`'s reply. The version has moved **twice** during v1.1 — Phase 14 took it 1 → 2, and the native wake ping took it 2 → 3 for the `SetWakeAddr` frame. Read the current value from `BUS_PROTO_VERSION` in `crates/famp-bus/src/proto.rs`, never from this paragraph. Every bump is a **hard, by-design break**: a client on an older protocol cannot render the new reply shapes, so the broker refuses it at Hello rather than silently serving unmarked remote content to a client blind to it — graceful degradation here would BE the vulnerability the quarantine boundary exists to close. Upgrading requires both steps above: reinstall the client (`cargo install --path crates/famp`, or `just install` from a contributor checkout) **and** `famp daemon restart` to pick up the new broker. Skipping either half fails loudly with a `ProtocolMismatch` error naming both remedies — it does not silently degrade. Full detail: [docs/MIGRATION-v1.0-to-v1.1.md](docs/MIGRATION-v1.0-to-v1.1.md).
...
setup. If you need cross-host federation (two machines you own), see
[docs/GATEWAY-SETUP.md](docs/GATEWAY-SETUP.md); for the
