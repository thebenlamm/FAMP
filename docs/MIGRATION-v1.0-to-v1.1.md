...
- Any foreign (non-`famp`) implementation of the local bus protocol, including a prior Grok interop session that has already wedged a mailbox once with a malformed envelope before this phase existed — expect a hard failure at Hello until it is updated to the shipping `bus_proto` and understands the new reply shape. **The value moved again after this section was written:** Phase 14 took it 1 → 2, and quick task 260810-hac took it 2 → 3 for the `SetWakeAddr` frame (see [`docs/superpowers/specs/2026-08-10-native-wake-ping-design.md`](superpowers/specs/2026-08-10-native-wake-ping-design.md)). **v1.1 ships `bus_proto: 3`** — read the current value from `BUS_PROTO_VERSION` in `crates/famp-bus/src/proto.rs`, never from this paragraph. The rejection mechanism described above is unchanged by the second bump.
