...
## The automatic-ingestion boundary (Phase 19, QUAR-12..14)
...
The broker appends all three origins to the recipient mailbox. It advances the
Await consumer past Gateway- and Unknown-origin records without returning them,
while an explicit Inbox read from an earlier offset can still retrieve those
records. Local-origin records retain the same automatic Await wake behavior as
before. The end-to-end proof is
`auto_wake_gate::remote_is_held_until_local_wakes_and_remains_in_inbox`.

This is an automatic Await eligibility gate, not held-by-default ingress.
Remote traffic still enters and can grow the mailbox, explicit reads still
return it, and a local agent can quote or paraphrase it into a newly Local-origin
message as described by the one-hop laundering limitation below. Broker Await
is also upstream of host UI/model re-entry: the gate proves which records can
produce an Await result, while host adapters and any other re-entry mechanisms
...
- **All seven rendering surfaces marked through one helper.** `famp_inbox`, `famp_await`, `famp_channel_log`, CLI `inbox list`, CLI `await`, CLI `register --tail`, and CLI `wait-reply` all reach `famp::cli::render::render_envelope_body`/`render_body_text` — the one shared implementation (D-07), not N ad-hoc copies. Rendering coverage remains in `crates/famp/tests/quarantine_surfaces.rs`, including `await_leaves_local_origin_verbatim`, `wait_reply_marks_gateway_origin`, `register_tail_marks_gateway_origin`, and `channel_log_marks_gateway_origin`; remote Await eligibility and explicit-Inbox visibility are proven separately by `auto_wake_gate::remote_is_held_until_local_wakes_and_remains_in_inbox`.
...
## Other boundaries remain separate

Phase 19 adds the broker-owned automatic Await eligibility boundary described
above. Provenance remains the prerequisite for separate defenses at other
boundaries; before Phase 14, none of them had a reliable signal to act on,
because `strip_relay_fields` erased every trace of remote origin before the
local bus ever saw it.
